...
const MapPopup = ({ name, count }: MapPopupProps) => (
    <div
        className="bg-white p-3 space-y-1 rounded-lg"
        style={{
            boxShadow:
...
export const UsaMapIcon = ({ pathFills = {}, redirect, className }: UsaMapProps) => {
    ...
    const getStateFill = (stateId: string, group: string, isHovered: boolean): string => {
        ...
        const fillEntry = pathFills[stateId] || pathFills[group];
        if (typeof fillEntry === "string") return fillEntry;
        if (fillEntry && typeof fillEntry === "object") return fillEntry.color;
        const stateData = STATE_DATA.find((s) => s.id === stateId);
        return stateData?.defaultColor || "#D0D0D0";
    };

    const getStateCount = (stateId: string, group: string): number => {
        const fillEntry = pathFills[stateId] || pathFills[group];
        if (fillEntry && typeof fillEntry === "object" && fillEntry.value)
            return fillEntry.value;
                ...
                <g className="state">
                    ...
                    <path
                        ...
                        onMouseEnter={(e) => {
                            ...
                            if (state) handleMouseEnter(e, state);
                        }}
